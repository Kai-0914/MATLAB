...
%% constant
f1 = 2.4*10^9; % frequency 1
f2 = 5.8*10^9; % frequency 2
c = 3*10^8; % lightspeed
beta1 = 2*pi*f1/c; % beta at frequency 1
beta2 = 2*pi*f2/c; % beta at frequency 2
...
I1_topX = 0; I1_topY = 8; I1_topZ = 2; % first image

reflect1_topY = 6; % reflected ray
reflect1_topZ = 2;
reflect1_topX = (reflect1_topY - I1_topY) ./ ((receiverY - I1_topY) ./ (receiverX - I1_topX));

% first order coordinate (bottom)
I1_bottomX = 0; I1_bottomY = -4; I1_bottomZ = 2; % first image

reflect1_bottomY = 0; % reflected ray
reflect1_bottomZ = 2;
...
reflect2b_topY = 0; % reflected ray
reflect2b_topZ = 2;
...
reflect2a_bottomY = 0; % reflected ray
reflect2a_bottomZ = 2;
...
intersect_bottomX = 7.7; % intersection when connect a imaginary line from source to receiver
intersect_bottomY = 6;
intersect_bottomZ = 2;
...
reflect3c_topY = 6; % reflected ray
reflect3c_topZ = 2;
...
intersect3a_topX = 2.19; % intersection when connect a imaginary line from source to receiver
...
reflect3c_bottomY = 0; % reflected ray
reflect3c_bottomZ = 2;
...
intersect3a_bottomX = 4; % intersection when connect a imaginary line from source to receiver
...
reflect4d_topY = 0;  % reflected ray
reflect4d_topZ = 2;
...
intersect4a_topX = 1.4; % intersection when connect a imaginary line from source to receiver
...
reflect4d_bottomY = 6;  % reflected ray
reflect4d_bottomZ = 2;
...
intersect4a_bottomX = 2.96; % intersection when connect a imaginary line from source to receiver
...
A1_top = sqrt(((sourceX - reflect1_topX).^2)+((sourceY - reflect1_topY)^2)+((sourceZ - reflect1_topZ)^2)); % imaginary triangle to find angle of relfection
...
theta1_top = acosd((A1_top^2 + B1_top^2 - C1_top^2) / (2 * A1_top * B1_top)); % find angle, incident angle, reflected angle
theta1_top_i = theta1_top ./ 2;
theta1_top_t = asind((1/sqrt(3)) * sind(theta1_top_i));

coefficient1_top = (cosd(theta1_top_i) - (sqrt(3) * cosd(theta1_top_t))) / (cosd(theta1_top_i) + (sqrt(3) * cosd(theta1_top_t))); % finding reflecting coefficient

E1_top_1 = ((1./distance1_top).*exp(-j * beta1 * distance1_top)) * coefficient1_top; % finding E-field
...
A1_bottom = sqrt(((sourceX - reflect1_bottomX).^2)+((sourceY - reflect1_bottomY)^2)+((sourceZ - reflect1_bottomZ)^2)); % imaginary triangle to find angle of relfection
...
theta1_bottom = acosd((A1_bottom^2 + B1_bottom^2 - C1_bottom^2) / (2 * A1_bottom * B1_bottom)); % find angle, incident angle, reflected angle
theta1_bottom_i = theta1_bottom ./ 2;
theta1_bottom_t = asind((1/sqrt(3)) * sind(theta1_bottom_i));

coefficient1_bottom = (cosd(theta1_bottom_i) - (sqrt(3) * cosd(theta1_bottom_t))) / (cosd(theta1_bottom_i) + (sqrt(3) * cosd(theta1_bottom_t))); % finding reflecting coefficient

E1_bottom_1 = ((1./distance1_bottom).*exp(-j * beta1 * distance1_bottom)) * coefficient1_bottom; % finding E-field
E1_bottom_2 = ((1./distance1_bottom).*exp(-j * beta2 * distance1_bottom)) * coefficient1_bottom;

% total E-field and convert to decibels
...
%% second order
% top
distance2_top = sqrt(((PtoQ - I2b_topX).^2)+((receiverY - I2b_topY)^2)+((receiverZ - I2b_topZ)^2)); % distance along x-axis 

A2a_top = sqrt(((sourceX - reflect2a_topX).^2)+((sourceY - reflect2a_topY)^2)+((sourceZ - reflect2a_topZ)^2)); % imaginary triangle to find angle of relfection
...
theta2a_top = acosd((A2a_top^2 + B2a_top^2 - C2a_top^2) / (2 * A2a_top * B2a_top)); % find angle, incident angle, reflected angle
...
coefficient2a_top = (cosd(theta2a_top_i) - (sqrt(3) * cosd(theta2a_top_t))) / (cosd(theta2a_top_i) + (sqrt(3) * cosd(theta2a_top_t))); % finding reflecting coefficient
coefficient2b_top = (cosd(theta2b_top_i) - (sqrt(3) * cosd(theta2b_top_t))) / (cosd(theta2b_top_i) + (sqrt(3) * cosd(theta2b_top_t)));

E2_top_1 = ((1./distance2_top).*exp(-j * beta1 * distance2_top)) * (coefficient2a_top * coefficient2b_top); % finding E-field
E2_top_2 = ((1./distance2_top).*exp(-j * beta2 * distance2_top)) * (coefficient2a_top * coefficient2b_top);

% bottom
distance2_bottom = sqrt(((PtoQ - I2b_bottomX).^2)+((receiverY - I2b_bottomY)^2)+((receiverZ - I2b_bottomZ)^2)); % distance along x-axis 

A2a_bottom = sqrt(((sourceX - reflect2a_bottomX).^2)+((sourceY - reflect2a_bottomY)^2)+((sourceZ - reflect2a_bottomZ)^2)); % imaginary triangle to find angle of relfection
...
theta2a_bottom = acosd((A2a_bottom^2 + B2a_bottom^2 - C2a_bottom^2) / (2 * A2a_bottom * B2a_bottom)); % find angle, incident angle, reflected angle
...
coefficient2a_bottom = (cosd(theta2a_bottom_i) - (sqrt(3) * cosd(theta2a_bottom_t))) / (cosd(theta2a_bottom_i) + (sqrt(3) * cosd(theta2a_bottom_t))); % finding reflecting coefficient
...
E2_bottom_1 = ((1./distance2_bottom).*exp(-j * beta1 * distance2_bottom)) * (coefficient2a_bottom * coefficient2b_bottom); % finding E-field
...
%% third order
% top
distance3_top = sqrt(((PtoQ - I3c_topX).^2)+((receiverY - I3c_topY)^2)+((receiverZ - I3c_topZ)^2)); % distance along x-axis 

A3a_top = sqrt(((sourceX - reflect3a_topX).^2)+((sourceY - reflect3a_topY)^2)+((sourceZ - reflect3a_topZ)^2)); % imaginary triangle to find angle of relfection
...
theta3a_top = acosd((A3a_top^2 + B3a_top^2 - C3a_top^2) / (2 * A3a_top * B3a_top)); % find angle, incident angle, reflected angle
...
coefficient3a_top = (cosd(theta3a_top_i) - (sqrt(3) * cosd(theta3a_top_t))) / (cosd(theta3a_top_i) + (sqrt(3) * cosd(theta3a_top_t))); % finding reflecting coefficient
...
E3_top_1 = ((1./distance3_top).*exp(-j * beta1 * distance3_top)) * (coefficient3a_top * coefficient3b_top * coefficient3c_top); % finding E-field
E3_top_2 = ((1./distance3_top).*exp(-j * beta2 * distance3_top)) * (coefficient3a_top * coefficient3b_top * coefficient3c_top);

% bottom
distance3_bottom = sqrt(((PtoQ - I3c_bottomX).^2)+((receiverY - I3c_bottomY)^2)+((receiverZ - I3c_bottomZ)^2)); % distance along x-axis 

A3a_bottom = sqrt(((sourceX - reflect3a_bottomX).^2)+((sourceY - reflect3a_bottomY)^2)+((sourceZ - reflect3a_bottomZ)^2)); % imaginary triangle to find angle of relfection
...
theta3a_bottom = acosd((A3a_bottom^2 + B3a_bottom^2 - C3a_bottom^2) / (2 * A3a_bottom * B3a_bottom)); % find angle, incident angle, reflected angle
...
coefficient3a_bottom = (cosd(theta3a_bottom_i) - (sqrt(3) * cosd(theta3a_bottom_t))) / (cosd(theta3a_bottom_i) + (sqrt(3) * cosd(theta3a_bottom_t))); % finding reflecting coefficient
...
E3_bottom_1 = ((1./distance3_bottom).*exp(-j * beta1 * distance3_bottom)) * (coefficient3a_bottom * coefficient3b_bottom * coefficient3c_bottom); % finding E-field
...
%% fourth order
% top
distance4_top = sqrt(((PtoQ - I4d_topX).^2)+((receiverY - I4d_topY)^2)+((receiverZ - I4d_topZ)^2)); % distance along x-axis 

A4a_top = sqrt(((sourceX - reflect4a_topX).^2)+((sourceY - reflect4a_topY)^2)+((sourceZ - reflect4a_topZ)^2)); % imaginary triangle to find angle of relfection
...
theta4a_top = acosd((A4a_top^2 + B4a_top^2 - C4a_top^2) / (2 * A4a_top * B3a_top)); % find angle, incident angle, reflected angle
...
coefficient4a_top = (cosd(theta4a_top_i) - (sqrt(3) * cosd(theta4a_top_t))) / (cosd(theta4a_top_i) + (sqrt(3) * cosd(theta4a_top_t))); % finding reflecting coefficient
...
E4_top_1 = ((1./distance4_top).*exp(-j * beta1 * distance4_top)) * (coefficient4a_top * coefficient4b_top * coefficient4c_top * coefficient4d_top); % finding E-field
...
% bottom
distance4_bottom = sqrt(((PtoQ - I4d_bottomX).^2)+((receiverY - I4d_bottomY)^2)+((receiverZ - I4d_bottomZ)^2)); % distance along x-axis 

A4a_bottom = sqrt(((sourceX - reflect4a_bottomX).^2)+((sourceY - reflect4a_bottomY)^2)+((sourceZ - reflect4a_bottomZ)^2)); % imaginary triangle to find angle of relfection
...
theta4a_bottom = acosd((A4a_bottom^2 + B4a_bottom^2 - C4a_bottom^2) / (2 * A4a_bottom * B3a_bottom)); % find angle, incident angle, reflected angle
...
coefficient4a_bottom = (cosd(theta4a_bottom_i) - (sqrt(3) * cosd(theta4a_bottom_t))) / (cosd(theta4a_bottom_i) + (sqrt(3) * cosd(theta4a_bottom_t))); % finding reflecting coefficient
...
E4_bottom_1 = ((1./distance4_bottom).*exp(-j * beta1 * distance4_bottom)) * (coefficient4a_bottom * coefficient4b_bottom * coefficient4c_bottom * coefficient4d_bottom); % finding E-field
...
hold on; % draw multiple line in a single graph
plot(PtoQ, EddB_1, 'b');
plot(PtoQ, E1dB_1, 'r');
plot(PtoQ, E2dB_1, 'k');
plot(PtoQ, E3dB_1, 'm');
...
subplot(2,1,2); % creating a subplot
hold on; % draw multiple line in a single graph
plot(PtoQ, EddB_2, 'b');
plot(PtoQ, E1dB_2, 'r');
plot(PtoQ, E2dB_2, 'k');
